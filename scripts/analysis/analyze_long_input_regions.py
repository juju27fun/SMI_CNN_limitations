...
def main() -> None:
    ...
    fingerprint = hashlib.sha256(json.dumps(provenance, sort_keys=True, separators=(",", ":")).encode()).hexdigest(); outputs = sorted(path.name for path in output.iterdir() if path.is_file() and path.name != "run.json")
