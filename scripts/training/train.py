...
class AdaptiveBandpassDecimate:
    """Bandpass + stride-decimate, single-pass, with dynamic anti-alias cutoff.

    Simulates what a lower-sample-rate ADC would capture: the upper cutoff is
    lowered to 0.9 * Nyquist_post_decimation whenever the fixed 100 kHz
    passband upper bound would violate Nyquist after striding. The low cutoff
    is unchanged (5 kHz is well below any Nyquist we sweep).

    Parameters
    ----------
    target_length : int
        Desired signal length after decimation (e.g. 512, 1024, ..., 16384).
    native_length : int, default 16384
        Length of the input signal before decimation.
    native_fs_hz : float, default 2_000_000
        Native sampling frequency in Hz (2 MHz in this project).
    low_khz : float, default 5.0
        Lower bandpass cutoff (kHz), always applied.
    high_khz_max : float, default 100.0
        Upper bandpass cutoff cap (kHz). The effective cutoff is
        ``min(high_khz_max, 0.9 * Nyquist_post_decimation)`` in kHz.
    """

    def __init__(self, target_length: int, native_length: int = 16384,
                 native_fs_hz: float = 2_000_000.0,
                 low_khz: float = 5.0, high_khz_max: float = 100.0):
...
class CenterCrop:
    """Crop a fixed-length window from the centre of the signal.

    If the signal is shorter than ``length``, it is zero-padded symmetrically.
    """
    def __init__(self, length: int):
        self.length = length
...
def run_training(args, device, class_names, train_loader, val_loader, test_loader):
    ...
    # Detect actual input length from one sample (works for any signal length)
    sample, _ = train_loader.dataset[0]
    input_length = sample.shape[-1]  # after transforms (bandpass + decimate)

    model = create_model(args.model, input_length=input_length, num_classes=len(class_names)).to(device)
...
def main():
    ...
    run_name = f"{args.model}-{data_dir.name}-train"
    ...
    # Adaptive batch size: probe GPU memory with a trial forward+backward pass
    if device.type == "cuda":
        sample, _ = train_dataset[0]
        input_length = sample.shape[-1]
        probe_model = create_model(args.model, input_length=input_length,
                                   num_classes=len(class_names)).to(device)
        probe_model.train()
        probe_opt = torch.optim.Adam(probe_model.parameters())
        effective_bs = args.batch_size
        for bs in sorted({args.batch_size, args.batch_size // 2, 8, 4, 2},
                         reverse=True):
            if bs < 1:
                continue
            try:
                torch.cuda.empty_cache()
                x = torch.randn(bs, 1, input_length, device=device)
                loss = probe_model(x).sum()
                loss.backward()
                probe_opt.step()
                probe_opt.zero_grad()
                del x, loss
        ...
        if effective_bs != args.batch_size:
            print(f"Batch size adapted: {args.batch_size} -> {effective_bs} (GPU memory)")
